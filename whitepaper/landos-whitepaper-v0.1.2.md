...
Landos is built around the individual human — not a household, a family, a role, or a gender. Every person who registers gets their own DID. Co-ownership is always a list of named individuals, never a "head of household." This is a direct response to the most common pattern in customary tenure: household land recorded under one person, almost always the man of the house, leaving everyone else in that household without a recognized claim of their own.
...
### 4.4 Validation Without a Social Graph

The five mechanisms above are a menu, not a checklist. A claim does not need all five — it needs the combined Proof of Land score to clear its floors, using whatever evidence is actually available to that particular claimant.

This matters because requiring Neighbor Quorum specifically would fail exactly the people Landos exists for: someone newly arrived with no local relationships yet, someone in a sparse area with no neighbors at all, or — the case closest to de Soto's framing — someone whose neighbor is a more powerful actor who simply will not attest for them.

**The Independent Claimant Path.** Registration itself has no prerequisites. Anyone can create a Parcel Record and enter the status spectrum at Claimed with zero attestations. From there, a claimant with no usable social graph leans on the mechanisms that don't depend on one — Habitation and AR Boundary Walking — while Neighbor Quorum is skipped rather than blocking progress.

**Land Witness as fallback for Neighbor Quorum.** When Neighbor Quorum cannot be formed — no neighbors exist, none are reachable, or the only relevant neighbor is the party the claimant is in conflict with — Land Witness review (Section 5, Stage 4) can substitute for it. Because this removes one of the staged entry's independent signals, the substitution carries a higher bar than routine Stage 4 review: two independent Land Witnesses, rather than one, plus an extended Habitation minimum for that claim, calibrated during testnet.

**Why this isn't a loophole.** Skipping Neighbor Quorum removes a requirement to gather proactive corroboration — it does not remove anyone's ability to object. The Notice Period stays hub-visible regardless of which path a claim entered through, so a real neighbor who was never asked to attest can still raise a Contest. Easier to start, still fully contestable — the same asymmetry the staged entry model already relies on.
...
zkSNARKs are production-ready as of 2026, widely deployed in financial privacy applications, and well-suited to the identity and location verification requirements of Landos. Four purpose-built proofs cover the points in the protocol where private data would otherwise have to touch the public chain:

**Location Proof.** Instead of publishing GPS coordinates, a claimant proves that their sensor readings placed them inside their registered parcel's boundary — a point-in-polygon check run entirely inside the circuit. The chain sees a single valid/invalid result. No coordinates, no boundary shape, no sensor data ever appear on-chain.

**Identity Proof.** Instead of publishing a verified-identity credential, a claimant proves that they hold a valid, unexpired credential signed by an approved provider, bound to their own DID. The chain sees "verified unique human from an approved provider" — never a name, a government ID, or which provider was used.

**Attestation Proof.** Instead of publishing which neighbors signed a quorum, the protocol proves that a valid M-of-N quorum was met — signers in good standing, diversity rule satisfied, no one attesting twice — without revealing any signer's identity. This is what keeps the social graph off the public chain entirely: an observer who watches every attestation on Landos still cannot reconstruct who knows whom.

**Score Proof.** Instead of publishing a running habitation score (which would let an observer build an activity calendar for any parcel), the protocol proves only that a DID's score remains above the confirmation floor. The score itself, and every check-in behind it, stays private. When a dispute genuinely requires more detail, a claimant can generate a proof directed specifically at a Land Witness — never published on-chain — so scrutiny is possible without making everyone's presence history public by default.

Identity verification itself is handled by Persona (initial partner) using W3C Verifiable Credentials, with the provider list itself governed on-chain so no single provider is a single point of failure.

### 6.4 DID Role Separation

A DID is pseudonymous, not anonymous — every action tied to one accumulates into a profile, and connecting a DID to a real person once exposes that entire history. Landos limits this by using one DID per role rather than one DID per person:

- **Landowner DID** — land claims, ownership, and transfers
- **Validator DID** — attestations, quorum signatures, and Land Witness certifications
- **Governance DID** — voting in the Token Holder Assembly and hub governance

An adversary watching the validator attestation graph cannot automatically connect it to a person's land ownership record, because the profiles don't overlap. This layer works today, without waiting on the ZK proofs above, and is fully compatible with them.

### 6.5 Location Hashing

Parcel Records store a hash of the Coords URI, not the URI itself — a node sees an opaque identifier, never a readable location. The hash is salted to prevent rainbow-table attacks, and the salt is derived mathematically from the user's own private key: not stored separately, not held by the Foundation, not controlled by any third party. Whoever holds their key already holds their salt. No new place for that secret to leak from.
...
**DID (Decentralized Identifier)** — a W3C self-sovereign identifier that no central authority controls. Landos uses one DID per role rather than one per person — a Landowner DID (claims, ownership, transfers), a Validator DID (attestations, quorum signatures, Land Witness certifications), and a Governance DID (voting) — so that activity in one role cannot be automatically linked to activity in another.
...
**Independent Claimant Path** — the ability to register land with zero prerequisites, entering the status spectrum at Claimed with no attestations; for claimants with no usable social graph, Land Witness review substitutes for Neighbor Quorum rather than blocking progress.
...
*Landos Whitepaper v0.1.2 — Draft. All sections subject to revision as design matures.*
