...
- Neighbor Quorum parameters (2026-09-13): confirmed at 3-of-5 for now, matching the design doc's existing default (Section 11.2). Note: the changelog's v0.1.6 entry claims this was set to 2-of-3 at that point, but the design doc text was never actually updated to match — 3-of-5 is what's live in v0.1.9 and what's reflected in the whitepaper. May be revisited during testnet calibration like other parameters ✓
...
2. **Hub boundary definition** — how geographic hub boundaries are drawn without political boundaries TBD
3. **Succession and inheritance** — DEFERRED; likely its own app and repo; requires legal review
4. **Privacy / identity layer** — zkSNARKs via gnark decided; full design session needed
5. **Coerced registration defense** — NEEDS LEGAL CONSULTATION before further design
6. **Fractional ownership edge cases** — disputes between co-owners, forced sale, deceased co-owner TBD
7. **The mobile app** — validation layer depends on an app not yet designed; offline/low-connectivity required
8. **LA_Restriction / LA_Responsibility representation** — liens, easements, conservation restrictions, and tax obligations now have a named category (Section 21.4); on-chain representation (separate token types vs. NFT metadata fields vs. other) TBD
9. **3D spatial unit design** — LADM Edition II Part 5 confirms the need (Section 21.5); Landos's own hectare-meter unit design remains deferred
10. **Land Witness capacity for the Independent Claimant Path** — requiring two independent Land Witnesses (Section 11.6) assumes enough Land Witnesses exist per hub to cover claimants with no social graph; how Land Witnesses are recruited/trained at scale is still open. Working direction (not yet specced): assignment, not search — eligible pool = hub members above a standing threshold who opt into an on-call roster; random draw filtered by Interested Party Exclusion (applied pairwise for the two-witness case); non-response reassigns rather than penalizes; shortfalls escalate to overlapping/adjacent hubs; pool eligibility bar starts low and rises with hub maturity, same soft-threshold pattern as validator bootstrapping. To be revisited during testing.
11. **Provisional governance process (pre-activation)** — the bicameral Token Holder/Community Hub Assembly design (Section 18.2) only makes sense once there's a real token distribution and real hubs; it says nothing about how decisions get made while Brock still holds admin keys and the first members are joining. Working direction (not yet specced): one-member-one-vote, not quadratic or token-weighted, since holdings aren't a meaningful signal of stake yet; a proposal gets a short notice window (echoing the 7-day staged-entry pattern) then a fixed voting window; each vote is a statement signed by the member's Governance DID, recorded in the community repo as the durable record — no live chain yet, but the same signing pattern carries forward once one exists; Brock retains override, but an override is logged with its reason rather than silently substituted. Two questions still open: (a) what qualifies someone as a "member" eligible to vote at this stage — Governance DID registration alone, or something more (a registered parcel, a direct invite)? (b) is simple majority the right bar, or should it be higher even at this small scale?
